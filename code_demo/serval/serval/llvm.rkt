...
(require (prefix-in core: "lib/core.rkt")
         ...
         (prefix-in unittest: "lib/unittest.rkt"))
...
; This is a global variable that tracks the current loop invariant as the loop body
; is being executed. We assume that there are no nested loops. This can easily be extended
; into a stack of values instead of a single value to hande nested loops.
(define cur-loop-inv #t)
...
; In our extended interpreter, return values are 2-tuples, where the second value
; containts conditions that need to hold for loop invariant to be preserved across iterations.
; As paths that return from the function do not go through back edges, they do not improse extra conditions.
; Thus, the preservation condition is simply "true" (#t) for these paths.
(define ret
  (case-lambda
    [() (cons void #t)]
    [(x) (cons x #t)]))
...
; Branch statements that start a loop are processed differently, through this interpreter.
(define br-loopstart
  (case-lambda
    [(test succ-true succ-false loop-inv modified-vars)
      (define mergepoint (current-merge-point))
      (define stack (current-merge-stack))

      (if (car mergepoint)
        (begin
          ; Our implementation currently does not care about state merging optimizations.
          ; Thus, this branch is left unmodified, and is never called.
          (set-machine-merge-stack! (current-machine) (cons (cdr mergepoint) stack))
          (if (core:bitvector->bool test)
            (enter! succ-true)
            (enter! succ-false))
        ...
        (begin
          ; BASE CASE: check if the loop invariant holds initially
          (printf "Does the loop invariant hold initially? ~v\n" (unsat? (verify (assert (loop-inv)))))

          ;;; Turn the local variables modified by the loop into unconstrained symbolic values.
          ;;; this installation of rosette does not allow for loops. We should express this through recursion.
          (define-symbolic* loop_var_prime (bitvector 32))

          ; for now, assume a single variable is modified so that we don't have to deal with Racket lists.
          (store loop_var_prime modified-vars (bitvector 32) #:align 4)


          ; assume the loop invariant
          ; assume doesn't work... No clue why. We just let Rosette pick arbitrary values and constrain the final query.
          ;;; (assume (loop-inv))
          (define loopinv-assumptions (loop-inv))
          (set! cur-loop-inv loop-inv)  ; used by br-backedge. Alternatively, we can pass the relevant loop invariant to the br-backedge interpreter.

          	
          ; We modify the ret instruction so that each returning path returns #t.
          ; Looping paths return the loop invariant over the post-loop state.
          ; So, the value of (if (test) succ-true succ-false) is a conjunction of loop invariant over the post-loop state across all looping paths.
          ; This conjunction is valid iff the loop invariant is preserved after an arbitrarty loop iteration.

          (define if-res (enter! (if (test) succ-true succ-false)))

          (define invariant-maintained (cdr if-res))
          (define return-value (car if-res))
          
          (printf "Is the loop invariant maintained? : ~v\n" (unsat? (verify (assert (=> loopinv-assumptions invariant-maintained)))))
...
(define br-backedge
  (case-lambda
    ...
    ; Do not let symbolic execution loop back. This path ends here, and is called a "looping path".
    ; The alternative are "exiting paths", which and at ret instructions.
    ; Return the loop invariant. The br-loopstart interpreter will check this.
    (cons "looping_path" (cur-loop-inv))
    ]

    ;;; [(test succ-true succ-false)
    ;;; ;;; we don't currently handle back edges in two-pronged branches.
    ;;; ]
    ))
