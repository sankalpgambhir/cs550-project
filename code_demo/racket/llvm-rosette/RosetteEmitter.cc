...
struct InstVisitor : llvm::InstVisitor<InstVisitor> {
    InstVisitor(const llvm::DataLayout &DL, llvm::raw_ostream &OS)
        : DL(DL), OS(OS), emit_loop(false) {}

    InstVisitor(const llvm::DataLayout &DL, llvm::raw_ostream &OS, bool loop)
        : DL(DL), OS(OS), emit_loop(loop) {}
...
void RosetteEmitter::emitBasicBlock(llvm::BasicBlock &BB, const llvm::PostDominatorTree &domtree)
{
    ...
    // If the bloc is labelled by LLVM as a for condition, then mark the next `br` as a loop
    bool is_loop = !strncmp(getName(BB).c_str(), "%for.cond", 9);
    OS << "\n  (define-label (" << getName(BB) << ")" << " #:merge " << merge;

    InstVisitor IV(BB.getModule()->getDataLayout(), OS, is_loop);
